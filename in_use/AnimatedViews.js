...
//how much of the adjacent items peek into the screen
const ITEM_PREVIEW = 10;
const ITEM_WIDTH = screen.width - (2 * ITEM_SPACING) - (2 * ITEM_PREVIEW);
//how far along pan snapping occurs -- this is the natural value
const SNAP_WIDTH = ITEM_WIDTH + ITEM_SPACING;
const ITEM_PREVIEW_HEIGHT = 100;
const SCALE_END = screen.width / ITEM_WIDTH;
const BREAKPOINT1 = 180;
const BREAKPOINT2 = 240;
...
function getMarkerState(panX, panY, scrollY, i) {
  ...
  //is this marker's preview the current index
  const isIndex = panX.interpolate({
    inputRange: [xRight - 1, xRight, xLeft, xLeft + 1],
    outputRange: [0, 1, 1, 0],
    extrapolate: 'clamp',
  });
  //is this marker's preview NOT the current index
  const isNotIndex = panX.interpolate({
    inputRange: [xRight - 1, xRight, xLeft, xLeft + 1],
    outputRange: [1, 0, 0, 1],
    extrapolate: 'clamp',
  });
  //is this marker's preview in the center
  ...
  //translate previews in y-direction - multiply just to extract the one currently selected
  const translateY = Animated.multiply(selected, panY);

  //translate previews in x-direction by however much user has panned
  const translateX = panX;

  //scale the preview size once past a certain point of scrollY
  const scale = Animated.add(ONE, Animated.multiply(selected, scrollY.interpolate({
  ...
  //make markers more transparent until a certain point of scrollY
...
class AnimatedViews extends React.Component {
  ...
  onPanYChange = ({ value }) => {
    ...
    //should be movable if panY less than 2 (really small)
    const shouldBeMovable = Math.abs(value) < 10;
    if (shouldBeMovable !== canMoveHorizontal) {
      this.setState({ canMoveHorizontal: shouldBeMovable });
      //if the window shouldn't be movable, then adjust the zoom of the map
      if (!shouldBeMovable) {
        ...
        region.timing({
          ...
          //decreasing deltas as we scroll closer to breakpoint
        ...
        //if the window should be movable, then resume allowing scrollX animations
      } else {
        region.stopAnimation();
        region.timing({
          latitude: scrollX.interpolate({
